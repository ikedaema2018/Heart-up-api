class LocateInfosController < ApplicationController
  ...
  def show
    ...
    if @locate_info[:user_id] == @user.id
      NayamiComment.where(locate_info_id: @locate_info[:id]).update_all(yonda_flag: true).order(id: "DESC")
    ...
    render json: @locate_info, include: ['nayami_comments', 'nayami_comments.user', 'user', 'first_locate']
  end
