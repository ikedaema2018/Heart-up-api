class LocateInfosController < ApplicationController
  ...
  def show
    ...
    if @locate_info[0][:user_id] == @user.id
      NayamiComment.where(locate_info_id: @locate_info[0][:id]).update_all(yonda_flag: true)
      if @locate_info[0][:life_flag] == true
        #取得したlocate_infoのsplash_yonda_checkがfalseだった時、そこのnayami_commentsを全部trueにしてsplash_yonda_flagもtrueに
        @splash_yonda_check = SplashYondaCheck.find_by(locate_info_id: @locate_info[0][:id])
